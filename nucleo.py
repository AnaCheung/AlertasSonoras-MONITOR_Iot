...
import config 
# LABORATORIO: monitor independiente del tráfico medido.
...
def iniciar(): 
    ...
    ahora = time.time() 
    # LABORATORIO: inicializar flanco y reproductor.
    estado_red = conexion.iniciar()
    alertas.iniciar()
    if not estado_red:
        eventos.atender("red_desconectada", {"conectado": False})
...
def ciclo(): 
    ...
    # LABORATORIO: supervisión del enlace en cada vuelta sin tocar sensores.
    for nombre, dato in conexion.revisar():
        nuevos.append(eventos.atender(nombre, dato))

    # LABORATORIO: alertas persistentes se recuerdan sin bloquear el ciclo.
    from alertas import reproductor
    reproductor.recordar()
